    ...
    def list_candidates(self) -> list[Outfit]:
        return [
            ...
            _mk("Embroidered Kurta", "FabIndia", "₹2,200", "top", ["beige", "gold"], ["ethnic", "festive"],
                 ...
                 trend=0.80, tier="mid", culture="indian"),
            _mk("Silk Kurta Set", "Manyavar", "₹4,500", "top", ["navy", "gold"], ["festive", "wedding"],
                 ...
                 trend=0.85, tier="premium", culture="indian"),
            _mk("Banarasi Silk Saree", "FabIndia", "₹8,500", "top", ["deep-green", "gold"], ["festive", "wedding"],
                 ...
                 trend=0.90, tier="premium", culture="indian"),
            _mk("Printed Cotton Kurta", "W", "₹1,200", "top", ["olive", "white"], ["casual", "everyday"],
                 ...
                 trend=0.70, tier="budget", culture="indian"),
            ...
            _mk("Oversized Boxy Tee", "Ader Error", "₹3,800", "top", ["white"], ["korean", "oversized"],
                 ...
                 trend=0.88, tier="mid", culture="korean"),
            _mk("Layered Shirt Jacket", "Gentle Monster", "₹5,500", "top", ["beige", "stone"], ["korean", "layered"],
                 ...
                 trend=0.82, tier="premium", culture="korean"),
            _mk("Cropped Cardigan", "Cos", "₹3,200", "top", ["pastel-blue", "white"], ["korean", "soft"],
                 ...
                 trend=0.85, tier="mid", culture="korean"),
            ...
            _mk("Embroidered Kaftan", "Raishma", "₹6,500", "top", ["white", "gold"], ["modest", "elegant"],
                 ...
                 trend=0.80, tier="premium", culture="middle_eastern"),
            _mk("Structured Abaya", "Hanayen", "₹5,800", "top", ["black", "gold"], ["modest", "formal"],
                 ...
                 trend=0.78, tier="premium", culture="middle_eastern"),
            _mk("Linen Thobe Tunic", "Marks & Spencer", "₹3,500", "top", ["white", "beige"], ["modest", "relaxed"],
                 ...
                 trend=0.72, tier="mid", culture="middle_eastern"),
            ...
            _mk("Nehru Jacket", "Raymond", "₹4,500", "top", ["navy", "gold"], ["fusion", "festive"],
                 ...
                 trend=0.85, tier="mid", culture="fusion"),
            _mk("Short Kurta", "FabIndia", "₹1,800", "top", ["white", "beige"], ["casual", "fusion"],
                 ...
                 trend=0.80, tier="mid", culture="fusion"),
            _mk("Draped Saree Gown", "Indya", "₹5,500", "top", ["deep-green", "gold"], ["fusion", "festive"],
                 ...
                 trend=0.88, tier="premium", culture="fusion"),
            _mk("Korean-Western Layered Blazer", "Cos", "₹6,200", "top", ["charcoal", "white"], ["korean", "fusion"],
                 ...
                 image="https://images.unsplash.com/photo-1490481651871-ab68de25d43d?w=1200&auto=format&sat=5",
                 trend=0.82, tier="premium", culture="fusion"),
            _mk("Modern Modest Tunic", "Aab", "₹3,800", "top", ["stone", "beige"], ["modest", "fusion"],
                 ...
                 image="https://images.unsplash.com/photo-1518310383802-640c2de311b2?w=1200&auto=format&sat=-5",
                 trend=0.75, tier="mid", culture="fusion"),
        ]
...
class OutfitScoringEngine:
    def score(self, candidates: Iterable[Outfit], ctx: ScoringContext) -> list[ScoredOutfit]:
        ...
        filtered = _apply_filters(filtered, desired_palette=desired_palette, vibe=ctx.vibe, occasion=occasion, culture=ctx.culture)
...
def _apply_filters(outfits: list[Outfit], desired_palette: list[str], vibe: str | None, occasion: str, culture: str | None = None) -> list[Outfit]:
    current = outfits

    # ── Culture filter (hard filter — first priority) ──
    if culture:
        culture_l = culture.strip().lower().replace(" ", "_").replace("-", "_")
        culture_filtered = [o for o in current if o.culture.lower().replace("-", "_") == culture_l]
        if culture_filtered:
            current = culture_filtered

    # ── Hard palette filter: reject outfits whose colors clash ──
    if desired_palette:
        compatible = [
            o for o in current
            if _palette_overlap(desired_palette, o.palette) > 0.0 and not _colors_clash(desired_palette, o.palette)
        ]
        # Only fall back to unfiltered if literally nothing matches (empty catalog edge case)
        if compatible:
            current = compatible

    # Vibe filter
    if vibe:
        vibe_l = vibe.strip().lower()
        vibe_filtered = [
            o
            for o in current
            if vibe_l in (t.lower() for t in o.tags) or vibe_l in _all_item_tags(o)
        ]
        if vibe_filtered:
            current = vibe_filtered

    # Occasion filter
    if occasion:
        occ_filtered = [o for o in current if _occasion_match(occasion, o) >= 0.5]
        if occ_filtered:
            current = occ_filtered
